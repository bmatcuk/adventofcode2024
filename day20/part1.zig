...
/// Each cheat is 2 spaces away:
///   8
///  6.7
/// 1.X.2
///  3.4
///   5
const cheats = [_]Point{
    .{.x = -2, .y = 0},
    .{.x = -1, .y = -1},
    .{.x = 0, .y = -2},
    .{.x = 1, .y = -1},
...
pub fn main() !void {
    ...
    var potential_cheats = std.AutoHashMap(Point, std.ArrayList(Point)).init(allocator);
    defer {
    ...
    while (!current.eq(end_point)) : (time += 1) {
        ...
        for (cheats) |cheat| {
            const next_point = current.add(cheat);
            if (puzzle.try_get(next_point)) |c| {
                ...
                if (c == '.' and track.unchecked_get(next_point) == null) {
                    ...
                    try entry.value_ptr.append(current);
                }
            }
        }
        ...
        if (potential_cheats.fetchRemove(current)) |cheats_to_here| {
            for (cheats_to_here.value.items) |cheat_start| {
                const cheat_time = track.unchecked_get(cheat_start).? + 2;
